...
function InteractionCard({ interaction, onEdit }: InteractionCardProps) {
  const representative = useQuery(api.representatives.getRepresentativeDetails, {
    ...
    <div className="bg-white shadow rounded-lg p-6 mb-4">
        ...
        <div>
          <div className="flex items-center space-x-2">
            <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium capitalize" style={{
              backgroundColor: interaction.outcome === "positive" ? "rgb(220 252 231)" : 
                             interaction.outcome === "negative" ? "rgb(254 226 226)" :
                             interaction.outcome === "neutral" ? "rgb(229 231 235)" : "rgb(243 244 246)",
              color: interaction.outcome === "positive" ? "rgb(22 101 52)" :
                    interaction.outcome === "negative" ? "rgb(153 27 27)" :
                    interaction.outcome === "neutral" ? "rgb(31 41 55)" : "rgb(55 65 81)",
            }}>
              {interaction.outcome}
            </span>
      ...
      {interaction.message_feedback && (
        <div className="mt-4 space-y-2">
          <h4 className="text-sm font-medium text-gray-900">Message Feedback</h4>
          ...
          {interaction.message_feedback.what_worked && interaction.message_feedback.what_worked.length > 0 && (
            <div>
              <p className="text-xs text-gray-500">What Worked</p>
              <ul className="list-disc list-inside text-sm text-gray-700">
                {interaction.message_feedback.what_worked.map((item, index) => (
...
export function InteractionList() {
  ...
  const [editingInteraction, setEditingInteraction] = useState<Doc<"interactions"> | undefined>();
  const [dateRange, setDateRange] = useState<{ start: number; end: number } | undefined>();
