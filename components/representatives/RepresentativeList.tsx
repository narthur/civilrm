...
function RepresentativeCard({ representative, onEdit }: RepresentativeCardProps) {
  return (
    <div className="bg-white shadow rounded-lg p-6 mb-4">
      <div className="flex justify-between items-start">
        <div>
          <h3 className="text-lg font-medium text-gray-900">{representative.name}</h3>
          <p className="text-sm text-gray-500">{representative.title}</p>
          <p className="text-sm text-gray-500">{representative.office}</p>
          <div className="mt-2">
      ...
      {representative.notes && (
        <div className="mt-4">
          ...
          <p className="mt-1 text-sm text-gray-500">{representative.notes}</p>
        </div>
      )}
    </div>
  );
...
export function RepresentativeList() {
  ...
  const [editingRepresentative, setEditingRepresentative] = useState<Doc<"representatives"> | undefined>();

  const filteredRepresentatives = representatives?.filter(rep => 
    levelFilter === "all" || rep.level === levelFilter
  );
  ...
  return (
    <div>
      <div className="mb-6 flex justify-between items-center">
        <div>
          <label htmlFor="level-filter" className="mr-2 text-sm font-medium text-gray-700">
            Filter by level:
          </label>
          <select
            id="level-filter"
