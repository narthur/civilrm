...
export function RepresentativeForm({ representative, onClose, onSuccess }: RepresentativeFormProps) {
  const addRepresentative = useMutation(api.representatives.addRepresentative);
  const updateRepresentative = useMutation(api.representatives.updateRepresentative);
  ...
  const [formData, setFormData] = useState({
    ...
    communication_preferences: {
      preferred_style: representative?.communication_preferences?.preferred_style ?? "formal",
      key_interests: representative?.communication_preferences?.key_interests ?? [],
      best_practices: representative?.communication_preferences?.best_practices ?? [],
    },
  });
      ...
      <div className="bg-white rounded-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setFormData({ ...formData, level: e.target.value as "federal" | "state" | "local" })}
                  required
            ...
            <div>
              ...
              <textarea
                ...
                onChange={(e) =>
                  setFormData({
                    ...formData,
                    contactInfo: { ...formData.contactInfo, office_address: e.target.value },
                  })
                }
                rows={2}
                className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
          ...
          <div className="space-y-4">
            ...
            <div>
              <label htmlFor="preferred_style" className="block text-sm font-medium text-gray-700">
                Preferred Style
              </label>
              <select
                id="preferred_style"
          ...
          <div className="flex justify-end space-x-3 pt-5">
            ...
            >
              {isSubmitting ? "Saving..." : representative ? "Update" : "Create"}
            </button>
          </div>
        </form>
      </div>
